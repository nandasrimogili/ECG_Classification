...
# history = np.load('training_history2.npy', allow_pickle=True).item()
...
# Function to check if the image dimensions are consistent with an ECG image
def is_ecg_image(image):
    # Check if the image has a consistent aspect ratio
    aspect_ratio = image.width / image.height
    if aspect_ratio < 1.5 or aspect_ratio > 2:
        return False
    
    # Check if the image dimensions are within a certain range
    min_dimension = min(image.width, image.height)
    if min_dimension < 500 or min_dimension > 1000:
        return False
