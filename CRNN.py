...
# Define CNN-RNN model architecture
# model = Sequential([
...
#     MaxPooling2D((2, 2)),
#     TimeDistributed(Flatten()),  # Flatten features for each timestep
#     LSTM(128, return_sequences=True),  # LSTM layer with 128 units and return sequences
...
checkpoint_dir = "Model_Checkpoints3"

# Ensure the checkpoint directory exists
os.makedirs(checkpoint_dir, exist_ok=True)

# Define the checkpoint filename
checkpoint_path = os.path.join(checkpoint_dir, "model_4_epoch_{epoch:02d}.h5")
...
class SaveHistory(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        np.save(f'train_history_test3_epoch_{epoch+1}.npy', self.model.history.history)
